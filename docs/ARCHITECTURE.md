...
- D1 共享工作区只有在独立 `ARCHLENS_WORKSPACE_TOKEN` 与 `ARCHLENS_WORKSPACE_WRITE_ENABLED=true` 同时配置时才可写；公开 Demo 默认关闭，当前不提供成员级权限。
...
当前案例数据是本地精选种子数据，MCP 公开 Demo 默认无鉴权，工作区默认使用浏览器本地存储，也可以导出为带数据集版本的 JSON 快照。来源 intake 已提供单案例/目录级抓取与解析入口，并新增可选 D1 证据登记和 operator-token 共享空间；成员级权限、邀请、自动入库和更复杂的配额治理仍属于 M4 后续阶段。
