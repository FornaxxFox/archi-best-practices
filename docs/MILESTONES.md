...
当前进展：来源 intake、目录级 pipeline、`dataset:audit`、D1 证据登记、审核状态迁移、D1 限流 bucket、顺序 ingest、用户自有工作流 manifest 校验、可移交 workspace snapshot 和显式发布候选门禁已完成首个可执行闭环；自动入库、团队权限和更复杂的配额治理仍未完成。
