...
- 提供 8 个只读工具，以及可读取的数据集、案例索引和研究工作流资源
- 可选 Bearer 鉴权和运行时限流配置已实现，公开 Demo 默认不启用

验收标准：外部 Agent 能稳定检索、获取和比较案例，并在错误时得到可定位的响应。当前 `1.1.0` 契约的本地工具调用、资源读取、错误路径和资料包路径已通过测试；远程 smoke、持久化限流和主流客户端实测仍是发布与生产化门禁。
