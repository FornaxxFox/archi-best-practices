...
- 提供 8 个只读工具、固定与按案例读取的资源，以及 4 个 MCP 原生研究提示
- 可选 Bearer 鉴权和运行时限流配置已实现，公开 Demo 默认不启用

验收标准：外部 Agent 能稳定检索、获取和比较案例，并在错误时得到可定位的响应。当前 `1.2.0` 契约的本地工具调用、资源读取、研究提示、错误路径和资料包路径已通过测试；远程 smoke、持久化限流和主流客户端实测仍是发布与生产化门禁。
